...
import numpy as np
from bank import TokenTuringMachineEncoder
...
class DocREModel(nn.Module):
    def __init__(self,
        ...
        # self.emb_size = emb_size coref
        self.block_size = block_size
        self.num_class = num_class
        # self.num_labels = num_labels
        self.mu_encoder = TokenTuringMachineEncoder(process_size=2, memory_size=200, input_dim=self.emb_size,
    ...
    def get_hrt(self, sequence_output, attention, hts, sent_pos, entity_pos, mention_pos, men_graphs, ne):
        ...
        for i in range(batch_size):
            ...
            update_count = torch.zeros(s_ne).to(device)
            h_indices = ht_i[:, 0]
            t_indices = ht_i[:, 1]
    ...
    def forward(self,
        ...
        hs_e, ts_e = self.get_hrt(sequence_output, attention, hts, sent_pos, entity_pos, mention_pos, men_graphs, ne)
        ...
        rel_logits = self.classifier(r_rep_e_3)
